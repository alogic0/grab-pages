...
extractBody str =
  let 
--      uniTree = universeTree (parseTree str)
--      title = flattenTree [x | x@(TagBranch "title" _ _) <- uniTree]
--      headerTree = [x | x@(TagBranch "head" _ _) <- uniTree]
--      contentTree =
--        [x | x@(TagBranch "div" [("class", "post-content")] _) <- uniTree]
      content =
        ...
        rmBranchByNameAtr "h3" ("id","comments") $
        rmBranchByNameAtr "div" ("id","sharing_email") $
        rmBranchByNameAtr "div" ("style","display:none")  $
        rmBranchByNameAtr "div" ("id","carousel-reblog-box") $
        rmBranchByNameAtr "ol" ("class","commentlist") $
        rmBranchByNameAtr "div" ("id","respond") $
        rmBranchByName "iframe" $
        rmBranchByName "script" $ parseTree str 
  in
--    header1 ++ title ++ header2 ++ content ++ footer
    content
